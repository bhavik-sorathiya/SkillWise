...
### Identity & Profiles
- **`users`**: Secure identity table. Stores `email` and a bcrypt-hashed `password`.
- **`user_profiles`**: Tracks aggregated user skills and career trajectories via a flexible `profile_data` JSON column.

### Document Management
- **`user_resumes`**: Metadata about uploaded resumes (file path, type, upload date).
- **`resume_analysis`**: Stores the raw JSON output from the Gemini API after a resume is parsed, mapped back to the specific resume and user.

### Interview Engine
- **`interview_sessions`**: The core state machine for a mock interview. Tracks active vs. completed status, max question counts, and duration.
